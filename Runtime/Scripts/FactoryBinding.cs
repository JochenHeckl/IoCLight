...
public class FactoryBinding<ProductType> : ITypeBinding
{
	...
	public object Resolve( IContainer container )
	{
		...
		{
			throw new InvalidOperationException( $"Invalid producer supplied to factory binding. {typeof( ProductType ).Name} can not be assigned from {methodInfo.ReturnType.Name}." );
		}

		var resolvedParameters = methodInfo.GetParameters()
